...
main (void) {
  ...
  int i, n, status, sd, ip_flags[4] = {0}, tcp_flags[8] = {0}, ip_opt_len, tcp_opt_len, frame_length;
  ssize_t bytes;
  ...
  ip_options = allocate_ustrmem (TEXT_STRINGLEN);
  tcp_options = allocate_ustrmem (TEXT_STRINGLEN);
  ...
  fprintf (stdout, "Padded IP option length (bytes): %d\n", ip_opt_len);
  ...
  fprintf (stdout, "Padded TCP option length (bytes): %d\n", tcp_opt_len);
  ...
  tcphdr.th_sum = 0;
  tcphdr.th_sum = tcp4_checksum (iphdr, tcphdr, tcp_options, tcp_opt_len, NULL, 0);
  ...
  frame_length = ETH_HDRLEN + IP4_HDRLEN + ip_opt_len + TCP_HDRLEN + tcp_opt_len;
  ...
  // IP options
  memcpy (ether_frame + ETH_HDRLEN + IP4_HDRLEN, ip_options, ip_opt_len);

  // TCP header
  memcpy (ether_frame + ETH_HDRLEN + IP4_HDRLEN + ip_opt_len, &tcphdr, TCP_HDRLEN);

  // TCP Options
  memcpy (ether_frame + ETH_HDRLEN + IP4_HDRLEN + ip_opt_len + TCP_HDRLEN, tcp_options, tcp_opt_len);
