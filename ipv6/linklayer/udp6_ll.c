...
main (void) {
  ...
  int i, n, status, udp_datalen, frame_length, sd;
  ssize_t bytes;
  ...
  iphdr.ip6_plen = htons (UDP_HDRLEN + udp_datalen);
  ...
  frame_length = ETH_HDRLEN + IP6_HDRLEN + UDP_HDRLEN + udp_datalen;
  ...
  // UDP header
  memcpy (ether_frame + ETH_HDRLEN + IP6_HDRLEN, &udphdr, UDP_HDRLEN);

  // UDP data
  memcpy (ether_frame + ETH_HDRLEN + IP6_HDRLEN + UDP_HDRLEN, udp_data, udp_datalen);
