...
// Values set for a SYN packet with a TCP Maximum Segment Size option.
...
main (void) {
  ...
  int i, n, status, tcp_optlen, frame_length, sd, tcp_flags[8] = {0};
  ssize_t bytes;
  char *interface, *target, *src_ip, *dst_ip;
  struct ip6_hdr iphdr;
  struct tcphdr tcphdr;
  uint8_t *tcp_options, src_mac[MAC_LEN] = {0}, *ether_frame;
  uint32_t seq;
  ...
  // TCP Options
  tcp_options[0] = 2u;  // Option kind 2 = maximum segment size
  tcp_options[1] = 4u;  // This option kind is 4 bytes long
  tcp_options[2] = 0x1u;  // Set maximum segment size to 0x100 = 256
  tcp_options[3] = 0x0u;
  tcp_optlen = 4;

  // Pad to the next 4-byte boundary.
  while ((tcp_optlen % 4) != 0) {
    tcp_options[tcp_optlen] = 0;
    tcp_optlen++;
  }
  fprintf (stdout, "Padded option length (bytes): %d\n", tcp_optlen);
  ...
  iphdr.ip6_plen = htons (TCP_HDRLEN + tcp_optlen);
  ...
  tcphdr.th_off = (TCP_HDRLEN + tcp_optlen) / 4;
  ...
  tcphdr.th_sum = 0;
  tcphdr.th_sum = tcp6_checksum (iphdr, tcphdr, tcp_options, tcp_optlen, NULL, 0);

  // Fill out Ethernet frame header.

  // Ethernet frame length = Ethernet header (MAC + MAC + Ethernet type) + Ethernet data (IP header + TCP header + TCP options)
  frame_length = ETH_HDRLEN + IP6_HDRLEN + TCP_HDRLEN + tcp_optlen;
  ...
  // TCP options
  memcpy (ether_frame + ETH_HDRLEN + IP6_HDRLEN + TCP_HDRLEN, tcp_options, tcp_optlen);
