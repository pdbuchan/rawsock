...
#define TEXT_STRINGLEN 255    // Maximum number of characters in a string
...
main (void) {
  ...
  int i, status, datagram_length, sd, tcp_flags[8] = {0}, tcp_datalen;
  ssize_t bytes;
  ...
  iphdr.ip6_plen = htons (TCP_HDRLEN + tcp_datalen);
  ...
  tcphdr.th_sum = 0;
  tcphdr.th_sum = tcp6_checksum (iphdr, tcphdr, NULL, 0, (uint8_t *) tcp_data, tcp_datalen);

  // Fill out datagram.

  // Datagram length = IPv6 header + TCP header + TCP data
  datagram_length = IP6_HDRLEN + TCP_HDRLEN + tcp_datalen;

  // IPv6 header
  memcpy (datagram, &iphdr, IP6_HDRLEN);

  // TCP header
  memcpy (datagram + IP6_HDRLEN, &tcphdr, TCP_HDRLEN);

  // TCP data
  memcpy (datagram + IP6_HDRLEN + TCP_HDRLEN, tcp_data, tcp_datalen);
