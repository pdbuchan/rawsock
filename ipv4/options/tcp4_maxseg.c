...
main (void) {
  ...
  int i, n, status, sd, ip_flags[4] = {0}, tcp_flags[8] = {0}, opt_len, frame_length;
  ssize_t bytes;
  char *interface, *target, *src_ip, *dst_ip;
  struct ip iphdr;
  struct tcphdr tcphdr;
  uint8_t *src_mac, *dst_mac, *ether_frame, *tcp_options;
  uint32_t seq;
  ...
  // TCP Options
  opt_len = 0;
  tcp_options[0] = 2u; opt_len++;  // Option kind 2 = maximum segment size
  tcp_options[1] = 4u; opt_len++;  // This option kind is 4 bytes long
  tcp_options[2] = 0x1u; opt_len++;  // Set maximum segment size to 0x100 = 256
  tcp_options[3] = 0x0u; opt_len++;

  // Pad to the next 4-byte boundary.
  while ((opt_len % 4) != 0) {
    tcp_options[opt_len] = 0;
    opt_len++;
  }
  ...
  frame_length = ETH_HDRLEN + IP4_HDRLEN + TCP_HDRLEN + opt_len;
  ...
  // TCP Options
  memcpy (ether_frame + ETH_HDRLEN + IP4_HDRLEN + TCP_HDRLEN, tcp_options, opt_len * sizeof (uint8_t));
