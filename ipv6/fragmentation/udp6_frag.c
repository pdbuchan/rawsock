...
#include <netinet/in.h>       // IPPROTO_UDP, IPPROTO_FRAGMENT, INET6_ADDRSTRLEN
...
main (void) {
  ...
  int i, n, status, udp_datalen, frame_length, sd, fragbufferlen;
  ...
  uint8_t *udp_data, *fragbuffer, *src_mac, *ether_frame;
  ...
  while ((n = fgetc (fi)) != EOF) {
    if (i >= (IP_MAXPACKET - IP6_HDRLEN - UDP_HDRLEN)) {
  ...
  fragbufferlen = UDP_HDRLEN + udp_datalen;
  ...
  // UDP header
  memcpy (fragbuffer, &udphdr, UDP_HDRLEN);

  // UDP data
  memcpy (fragbuffer + UDP_HDRLEN, udp_data, udp_datalen);
  ...
  for (i = 0; i < nframes; i++) {
    ...
    // Ethernet frame length = ethernet header (MAC + MAC + ethernet type) + IPv6 header + [fragment header] + fragment
    if (nframes == 1) {
