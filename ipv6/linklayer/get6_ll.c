...
main (void) {
  ...
  int i, n, status, frame_length, sd, tcp_flags[8] = {0}, tcp_datalen;
  ssize_t bytes;
  ...
  // Set TCP data.
  snprintf (url, HOSTNAME_LEN, "ipv6.google.com");
  ...
  frame_length = ETH_HDRLEN + IP6_HDRLEN + TCP_HDRLEN + tcp_datalen;
  ...
  // TCP data
  memcpy (ether_frame + ETH_HDRLEN + IP6_HDRLEN + TCP_HDRLEN, tcp_data, tcp_datalen);
