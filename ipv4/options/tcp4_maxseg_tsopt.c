...
main (void) {
  ...
  int i, n, c, status, sd, ip_flags[4] = {0}, tcp_flags[8] = {0}, nopt, *opt_len, buf_len, frame_length;
  ssize_t bytes;
  ...
  opt_len[0] = 0;
  tcp_options[0][0] = 2u; opt_len[0]++;  // Option kind 2 = maximum segment size
  tcp_options[0][1] = 4u; opt_len[0]++;  // This option kind is 4 bytes long
  tcp_options[0][2] = 0x1u; opt_len[0]++;  // Set maximum segment size to 0x100 = 256
  tcp_options[0][3] = 0x0u; opt_len[0]++;
