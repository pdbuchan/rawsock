...
main (void) {
  ...
  struct nd_router_solicit rshdr;
  ...
  fprintf (stdout, "Sending to IPv6 \"all routers\" multicast address: %s\n", target);
  freeaddrinfo (res);
  ...
  // Router Solicitation Header (ICMPV6 header)
  ...
  rshdr.nd_rs_hdr.icmp6_cksum = htons(0);

  // Reserved (32 bits): Must be set to zero.
  rshdr.nd_rs_reserved = htonl (0);
  ...
  uint8_t icmp_msg[sizeof (struct nd_router_solicit) + sizeof (icmp_data)];
  memset (icmp_msg, 0, sizeof (icmp_msg));
  memcpy (icmp_msg, &rshdr, sizeof (struct nd_router_solicit));
  memcpy (icmp_msg + sizeof (struct nd_router_solicit), icmp_data, sizeof (icmp_data));
