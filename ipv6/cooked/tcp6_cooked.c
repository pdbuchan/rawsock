...
// Send a "cooked" IPv6 TCP datagram via packet socket.
...
main (void) {
  ...
  int i, status, datagram_length, sd, tcp_flags[8] = {0};
  ssize_t bytes;
  ...
  // Payload length (16 bits): TCP segment length; here, just the TCP header.
  ...
  datagram_length = IP6_HDRLEN + TCP_HDRLEN;
