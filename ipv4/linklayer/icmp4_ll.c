...
main (void) {
  ...
  int i, n, status, icmp_datalen, sd, ip_flags[4] = {0}, frame_length;
  ssize_t bytes;
  char *interface, *target, *src_ip, *dst_ip;
  struct ip iphdr;
  struct icmp icmphdr;
  uint8_t src_mac[MAC_LEN] = {0}, *icmp_msg, *ether_frame;
  ...
  // ICMP message (ICMP header + ICMP data)
  memcpy (ether_frame + ETH_HDRLEN + IP4_HDRLEN, icmp_msg, ICMP_HDRLEN + icmp_datalen);
