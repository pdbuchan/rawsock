...
// This example puts data in a SYN segment. Real TCP peers may ignore
// or reject data sent before the connection is established.
...
main (void) {
  ...
  dst_options[0][2] = 4;  // Some unique, unpredictable 12-byte number
  dst_options[0][3] = 35;
  dst_options[0][4] = 229;
  dst_options[0][5] = 0;
  dst_options[0][6] = 79;
  ...
  fprintf (stdout, "Number of destination options: %d\n", dst_nopt);
  fprintf (stdout, "Total length of destination options, excluding 2-byte destination header and padding: %d\n", dst_opt_totlen);
  fprintf (stdout, "Total length of destination alignment padding and end-padding: %d\n", dst_optpadlen);
  ...
  // of all extension headers, and therefore in the fragmentable portion.
  fragbufferlen = dstlen + TCP_HDRLEN + tcp_datalen;
  ...
  if (dst_nopt > 0) {
    ...
    memcpy (fragbuffer + c, &dsthdr, DST_HDRLEN);
    c += DST_HDRLEN;
    indx += DST_HDRLEN;
    ...
    for (j = 0; j < dst_nopt; j++) {
      ...
      memcpy (fragbuffer + c, dst_options[j], dst_optlen[j]);
      c += dst_optlen[j];
      indx += dst_optlen[j];
    }
  ...
  // TCP header
  memcpy (fragbuffer + c, &tcphdr, TCP_HDRLEN);
  c += TCP_HDRLEN;

  // TCP data
  memcpy (fragbuffer + c, tcp_data, tcp_datalen);
  c += tcp_datalen;
