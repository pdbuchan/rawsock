...
main (void) {
  ...
  fragbufferlen = ATH_HDRLEN + auth_len + IP6_HDRLEN + hoplen + TCP_HDRLEN + tcp_datalen;
  ...
  c += ATH_HDRLEN;
  memcpy (fragbuffer + c, auth_data, auth_len);  // Authentication data
  c += auth_len;

  // Original IPv6 header
  memcpy (fragbuffer + c, &iphdr, IP6_HDRLEN);
  c += IP6_HDRLEN;
